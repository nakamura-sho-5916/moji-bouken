...
v0.4.0では、CSS AnimationとSVG主体の軽量な演出を追加しました。対象は正解・不正解、敵被弾、敵撃破、レベルアップ、宝箱、仲間、世界復興、ショップ購入、レアアイテム獲得です。スマートフォンのPWA利用を優先し、演出は主に`transform`と`opacity`で構成しています。`prefers-reduced-motion: reduce`ではアニメーション時間を最小化します。
...
# Production Audio v0.6.0

v0.6.0 adds original procedural production BGM and SFX. No external audio files are bundled, streamed, downloaded, or copied from third-party sources. The app generates compact Web Audio API patterns at runtime, so audio works offline in the PWA after the app shell is cached.
...
Large cues such as `level-up`, `world-recovery`, `companion-joined`, rare drops, and fanfares temporarily duck the current BGM and restore it after the cue. Correct answers play the correct cue before battle impact; incorrect answers play only the retry cue.

Development-only `/debug/audio` shows AudioContext state, current BGM, BPM, current step, active node count, ducking state, manual SFX/BGM controls, and recent events. Production builds do not expose this debug screen.
