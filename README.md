...
現在のbuildでは、アプリ機能の追加によりchunk size warningが出る場合があります。PWA生成とbuild自体は成功しています。既存機能を壊さないことを優先し、この段階では大規模な分割リファクタリングは行っていません。必要になった場合はReact.lazyやルート単位のコード分割を検討します。
